...
public class CameraPreview: CAPPlugin {
    ...
    @objc func setFlashMode(_ call: CAPPluginCall) {
        ...
        do {
            ...
            switch flashMode {
            case "off":
                // Torch disabled across iOS; map to traditional flash OFF
                try self.cameraController.setFlashMode(flashMode: .off)
                call.resolve()
                return
            case "on":
                // Torch removed across iOS; map to traditional flash ON
                try self.cameraController.setFlashMode(flashMode: .on)
                call.resolve()
                return
            case "auto":
                flashModeAsEnum = AVCaptureDevice.FlashMode.auto
            case "torch":
                // Torch removed across iOS; treat as traditional flash ON
                try self.cameraController.setFlashMode(flashMode: .on)
                call.resolve()
                return
            default: break
