...
extension CameraController {
    ...
    func getSupportedFlashModes() throws -> [String] {
        ...
        // Torch removed for iOS; do not advertise torch support
    ...
    func setFlashMode(flashMode: AVCaptureDevice.FlashMode) throws {
        ...
        if supportedFlashModes.contains(flashMode) {
            do {
                ...
                // For ultra-wide cameras, also set the device flash mode to enable the flash LED
                if device.deviceType == .builtInUltraWideCamera && device.hasFlash {
                    device.flashMode = flashMode
                    print("Set device flash mode to \(flashMode.rawValue) for ultra-wide camera")
                }
    ...
    // Torch removed for iOS; map to flash ON
    func setTorchMode() throws {
        ...
        guard let _ = currentCamera else {
            throw CameraControllerError.invalidOperation
        }
        try setFlashMode(flashMode: .on)
    }
    
    private func enableTorchOnDevice(_ device: AVCaptureDevice) throws {
        do {
            try device.lockForConfiguration()
            
            // Check if torch is supported on this specific camera
            if device.isTorchModeSupported(AVCaptureDevice.TorchMode.on) {
                device.torchMode = AVCaptureDevice.TorchMode.on
                print("Torch enabled on camera: \(device.deviceType.rawValue)")
            } else if device.isTorchModeSupported(AVCaptureDevice.TorchMode.auto) {
                device.torchMode = AVCaptureDevice.TorchMode.auto
                print("Torch set to auto on camera: \(device.deviceType.rawValue)")
            } else {
                device.torchMode = AVCaptureDevice.TorchMode.off
                print("Torch not supported on camera: \(device.deviceType.rawValue)")
            }
    ...
    func setFlashOrTorchMode() throws {
        var currentCamera: AVCaptureDevice?
        switch currentCameraPosition {
        case .front:
            currentCamera = self.frontCamera!
        ...
        // For ultra-wide camera, use traditional flash (short burst) instead of torch
        if device.deviceType == .builtInUltraWideCamera {
            print("Using traditional flash (short burst) for ultra-wide camera")
            try setFlashMode(flashMode: AVCaptureDevice.FlashMode.on)
        } else {
            // For other cameras, use torch (continuous lighting)
            print("Using torch mode (continuous) for main camera")
            try setTorchMode()
        }
    }
    
    func setFlashOrTorchOff() throws {
        var currentCamera: AVCaptureDevice?
        switch currentCameraPosition {
        case .front:
            currentCamera = self.frontCamera!
        ...
        // For ultra-wide camera, turn off traditional flash
        if device.deviceType == .builtInUltraWideCamera {
            print("Turning off traditional flash for ultra-wide camera")
            try setFlashMode(flashMode: AVCaptureDevice.FlashMode.off)
        } else {
            // For other cameras, turn off torch
            print("Turning off torch for main camera")
            try setTorchOff()
        }
    }
    
    // Torch removed for iOS; map to flash OFF
    func setTorchOff() throws {
        var currentCamera: AVCaptureDevice?
        switch currentCameraPosition {
        case .front:
            currentCamera = self.frontCamera!
        ...
        try setFlashMode(flashMode: .off)
    }
    ...
    private func switchToCamera(_ camera: AVCaptureDevice) throws {
        ...
        // Save current torch state before switching
        var wasTorchOn = false
        if let currentInput = self.rearCameraInput {
            ...
            if currentDevice.hasTorch && currentDevice.isTorchAvailable {
                wasTorchOn = (currentDevice.torchMode == .on)
            }
        }
        ...
        if captureSession.canAddInput(newCameraInput) {
            ...
            // Restore torch state on new camera
            if wasTorchOn && camera.hasTorch && camera.isTorchAvailable {
                do {
                    try camera.lockForConfiguration()
                    if camera.isTorchModeSupported(.on) {
                        camera.torchMode = .on
                    } else if camera.isTorchModeSupported(.auto) {
                        camera.torchMode = .auto
                    }
                    camera.unlockForConfiguration()
                } catch {
                    print("Failed to restore torch state: \(error)")
                }
            }
        } else {
            throw CameraControllerError.invalidOperation
