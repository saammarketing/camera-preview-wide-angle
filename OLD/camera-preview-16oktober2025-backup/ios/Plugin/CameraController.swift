...
extension CameraController {
    ...
    func captureImage(completion: @escaping (UIImage?, Error?) -> Void) {
		...
		// Attempt to ensure focus/exposure are stabilized before capture
		if let device = (self.currentCameraPosition == .rear ? self.rearCamera : self.frontCamera) {
			do {
				...
				// ignore focus/exposure setup errors and proceed to capture
			}
			// Briefly wait for AF/AE to settle (max ~200ms)
			var attempts = 0
			while (device.isAdjustingFocus || device.isAdjustingExposure) && attempts < 4 {
				usleep(50000) // 50ms
				attempts += 1
			}
		}
		...
		// Prefer quality over speed to allow better timing with flash
    ...
    // Attempt to switch the active rear camera to the ultra‑wide lens if available.
    // If ultra‑wide is not available, throw so caller can fallback to zoom-based approach.
    func switchToWideAngle() throws {
        guard let captureSession = self.captureSession, captureSession.isRunning else {
            throw CameraControllerError.captureSessionIsMissing
        }

        // Only meaningful for rear position
        guard currentCameraPosition == .rear else {
            throw CameraControllerError.invalidOperation
        }

        // Discover ultra‑wide device
        ...
        guard let ultra = discovery.devices.first else {
            // No ultra‑wide on this device
            throw CameraControllerError.noCamerasAvailable
        }

        let newInput = try AVCaptureDeviceInput(device: ultra)

        captureSession.beginConfiguration()
        // Remove existing rear input if present
        if let rearInput = self.rearCameraInput, captureSession.inputs.contains(rearInput) {
            captureSession.removeInput(rearInput)
        }

        if captureSession.canAddInput(newInput) {
            captureSession.addInput(newInput)
            self.rearCamera = ultra
            self.rearCameraInput = newInput
            // Keep logical position as rear
            self.currentCameraPosition = .rear
        } else {
            captureSession.commitConfiguration()
            throw CameraControllerError.invalidOperation
    ...
    // Switch back to the main wide rear camera if available
    func switchToMainWide() throws {
        guard let captureSession = self.captureSession, captureSession.isRunning else {
            throw CameraControllerError.captureSessionIsMissing
        }
        ...
        guard let wide = discovery.devices.first else {
            throw CameraControllerError.noCamerasAvailable
        }
    ...
    // Toggle between ultra‑wide and main wide for the rear camera
    func toggleWideAngle() throws {
        guard currentCameraPosition == .rear else {
            throw CameraControllerError.invalidOperation
        }
        if rearCamera?.deviceType == .builtInUltraWideCamera {
            try switchToMainWide()
        } else {
            try switchToWideAngle()
        }
    }
